...
const TourPlanCard = ({ tourPlan, onEdit, onDelete, isNew = false, onClick }: TourPlanCardProps) => {
  if (isNew) {
    return (
      <Card className="h-full cursor-pointer transition-all duration-300 hover:shadow-md hover:scale-[1.02] border-dashed border-2 border-gray-300 flex flex-col justify-center items-center bg-gray-50" onClick={onClick}>
  ...
  const imageUrl = tourPlan.image_path 
    ? `${import.meta.env.VITE_SUPABASE_URL}/storage/v1/object/public/tour_plan_images/${tourPlan.image_path}`
    : '/placeholder.svg';
